...
{%- macro build_room(area_id) -%}
  ...
  {%- for entity_id in entities -%}
    ...
    {%- endif -%}
  {%- endfor -%}
  {%- if readings.temperature is not none or readings.humidity is not none or readings.pm25 is not none or lighting.available -%}
    {{- dict(id=area_id, name=area_name(area_id), temperature=readings.temperature, humidity=readings.humidity, pm25=readings.pm25, controls=active_controls.items, lighting=dict(available=lighting.available, on=lighting.on, colors=lighting.colors)) | to_json -}}
  {%- else -%}
    {{- none | to_json -}}
  {%- endif -%}
{%- endmacro -%}
