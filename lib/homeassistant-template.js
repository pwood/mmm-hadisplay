      ...
      {%- if entity_id in control.entities and has_value(entity_id) -%}
        {%- if entity_id[:8] == 'climate.' -%}
          ...
          {%- if action in ['heating', 'preheating', 'cooling'] -%}
            {%- set target = state_attr(entity_id, 'target_temp_low') if action in ['heating', 'preheating'] else state_attr(entity_id, 'target_temp_high') -%}
            {%- if not is_number(target) -%}
              {%- set target = state_attr(entity_id, 'temperature') -%}
            {%- endif -%}
            {%- if is_number(target) -%}
              {%- set unit = state_attr(entity_id, 'temperature_unit') | default('', true) -%}
              {%- set normalized_action = 'heating' if action == 'preheating' else action -%}
              {%- set active_controls.items = active_controls.items + [dict(metric='temperature', action=normalized_action, target=target | float, unit=unit)] -%}
            {%- endif -%}
          {%- elif action == 'drying' and is_number(state_attr(entity_id, 'humidity')) -%}
            {%- set active_controls.items = active_controls.items + [dict(metric='humidity', action='drying', target=state_attr(entity_id, 'humidity') | float, unit='%')] -%}
          {%- endif -%}
        {%- elif entity_id[:11] == 'humidifier.' -%}
          {%- set action = state_attr(entity_id, 'action') -%}
          {%- set target = state_attr(entity_id, 'humidity') -%}
          {%- if action in ['humidifying', 'drying'] and is_number(target) -%}
            {%- set active_controls.items = active_controls.items + [dict(metric='humidity', action=action, target=target | float, unit='%')] -%}
          {%- endif -%}
        {%- endif -%}
      {%- endif -%}
    {%- endfor -%}
